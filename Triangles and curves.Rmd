...
knit: (function(input_file, encoding) {
 ...
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'Triangles and curves.html'))})
title: "Triangles and curves"
...
subplot(
  get_curve_plot(x, A, B),
  get_points_plot(rpoints_df) %>% layout(title = "")
)
```
...
{
  return(c(mean(x)-sqrt(-log(0.025)*8/length(x)), mean(x)+sqrt(-log(0.025)*8/length(x))))
}
```

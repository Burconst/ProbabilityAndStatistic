...
knit: (function(input_file, encoding) {
 ...
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'Hypotheses and Least Squares.html'))})
title: "Hypotheses and Least Squares"
...
```{r first1}
Val <- read.csv("./data/Hypotheses and Least Squares/data.csv",header = FALSE, sep = ',')
...
get_pv_pvalue <- function(Val) 
{
  ...
  k <- sqrt(apply((Val - matrix(rep(M, ncol(Val)), ncol = ncol(Val)))^2, sum, MARGIN = 1)/(ncol(Val) - 1))
...
get_pv_pvalue(Val)$pvalue[get_pv_pvalue(Val)$pv < qchisq(0.95, 4)]
```
...
get_plot(get_pv_pvalue(Val)$pvalue[get_pv_pvalue(Val)$pv < qchisq(0.95, 4)],F_)
```
...
```{r second, message=F}
data1_csv <- read.tcsv("./data/Hypotheses and Least Squares/data1.csv", sep = ',')[1:11,1:2]

acceleration <- function(t, a) 0.7 + a*(t + 1)/(t + 2) 

get_theta <-function(t, x) 
{
  X <- matrix(c(t^2/2 + t - (t+2)*log(t+2), t, rep(1,length(t))), ncol = 3, nrow = length(t))
...
get_x_t <- function(t, x) 
{
  ...
  x_t <- function(t) theta[1,1]*(t^2/2 + t - (t+2)*log(t+2)) + theta[2,1]*t + theta[3,1]
  return(x_t)
}
...
fn <- function(t, x) 
{
  ...
  Val1 <- matrix(c(0.85 - 3*log(3), 1, 1,
                  0.7 - log(3), 1, 0,

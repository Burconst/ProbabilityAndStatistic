...
```{r second, message=F}
data1_csv <- read.tcsv("./data/Hypotheses and Least Squares/data1.csv",
                       sep = ',')[1:11,1:2]
...
{
  X <- matrix(c(t^2/2 + t +0.35*t^2 - (t+2)*log(t+2), t, rep(1,length(t))),
              ncol = 3, nrow = length(t))
...
get_x_t <- function(t, x) 
{
  ...
  x_t <- function(t) theta[1,1]*(t^2/2 + t + 0.35*t^2 - (t+2)*log(t+2)) + theta[2,1]*t + theta[3,1]
  return(x_t)
}
...
fn <- function(t, x) 
{
  ...
  Val1 <- matrix(c(1.85 - 3*log(3), 1, 1,
                   1.7 - log(3), 1, 0,

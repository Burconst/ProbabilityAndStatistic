...
density_plot <- function(dens, a, b) 
{
  ...
  fig <- plot_ly() %>%
    ...
    layout(plot_bgcolor = 'rgb(227, 218, 245)', legend = list(orientation = 'h'),
           ...
           title = paste("Density of zeta:  M[zeta] =",as.character(round(mean_val,2)),", D[zeta] =",as.character(round(sd^2,2))))
  return(fig)
}
...
distr_func_plot <- function(distr_func, dens, a, b) 
{
  ...
  fig <- plot_ly() %>% 
    ...
    layout(plot_bgcolor = 'rgb(227, 218, 245)', legend = list(orientation = 'h'),
           yaxis = list(zeroline = F), xaxis = list(zeroline = F),
           title = paste("DF of zeta:  M[zeta] =",as.character(round(mean_val,2)),", D[zeta] =",as.character(round(sd^2,2))))
  return(fig)
}
